...
import { AlphaVantageService, DailyData } from '../financial-data-provider/alpha-vantage.service';
  ...
  async ingestDailyDataForSymbol(symbol: string): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        this.logger.error(
          `Unknown error ingesting daily data for symbol ${symbol}: ${JSON.stringify(error)}`
        );
      }
    }
  }
